...
def compute_embeddings(corpus, queries, model, max_len, batch_size, data_name=None, corpus_embedding_available=True, use_full_dataset=True, no_retrieval=False):
    ...
    _ARGS.corpus_embedding_available = corpus_embedding_available
    _ARGS.use_full_dataset = use_full_dataset
    _ARGS.data_name = data_name

    if no_retrieval: # condition for getting embeddings without splitting into corpus and queries
    # if this is true it does not check the other conditions.
        if os.path.isfile("data/embeddings/embedding_{}_{}.npy".format(data_name, model.split('/')[-1])):
            embeddings_queries = np.load("data/embeddings/embedding_{}_{}.npy".format(data_name, model.split('/')[-1]))
        else:
            embeddings_queries = get_embeddings_queries(_ARGS)
            np.save("data/embeddings/embedding_{}_{}.npy".format(data_name, model.split('/')[-1]), embeddings_queries)
        return embeddings_queries

    if corpus_embedding_available: # condition for using the entire model cards as corpus and using a separate test set as queries
        if os.path.isfile("data/embeddings/embedding_corpus_{}.npy".format(model.split('/')[-1])):
            embeddings_corpus = np.load("data/embeddings/embedding_corpus_{}.npy".format(model.split('/')[-1]))
            embeddings_queries = np.load("data/embeddings/embedding_queries_{}.npy".format(model.split('/')[-1]))
            if use_full_dataset:
                embeddings_corpus = np.concatenate([embeddings_corpus, embeddings_queries], axis=0)
        else:   
            print ("Corpus embedding not available. Please check again.")
            return None
        if os.path.isfile("data/embeddings/embedding_queries_{}_{}.npy".format(data_name, model.split('/')[-1])):
            embeddings_queries = np.load("data/embeddings/embedding_queries_{}_{}.npy".format(data_name, model.split('/')[-1]))
        else:
            embeddings_queries = get_embeddings_queries(_ARGS)
            np.save("data/embeddings/embedding_queries_{}_{}.npy".format(data_name, model.split('/')[-1]), embeddings_queries)
    
    else: # condition to test only on model cards with train test split as corpus and queries
        if os.path.isfile("data/embeddings/embedding_corpus_{}_{}.npy".format(data_name, model.split('/')[-1])):
            embeddings_corpus = np.load("data/embeddings/embedding_corpus_{}_{}.npy".format(data_name, model.split('/')[-1]))
            embeddings_queries = np.load("data/embeddings/embedding_queries_{}_{}.npy".format(data_name, model.split('/')[-1]))
        else:
            embeddings_corpus, embeddings_queries = get_embeddings(_ARGS)

            np.save("data/embeddings/embedding_corpus_{}_{}.npy".format(data_name, model.split('/')[-1]), embeddings_corpus)
            np.save("data/embeddings/embedding_queries_{}_{}.npy".format(data_name, model.split('/')[-1]), embeddings_queries)
...
#     model = MODEL[1]
#     if model == "BAAI/bge-large-en-v1.5": max_len = 512
#     else: max_len = 1024

#     data = pd.read_csv("data/new_data/llm_analysis/close_mitigations.csv")
#     risks = data["sentences"]

#     all_risks = []
#     for i,row in data.iterrows():
#         all_risks.append(row["sentences"])
#     print (len(all_risks))
#     all_risks_embeddings = compute_embeddings(
#     corpus=None,
#     queries=all_risks,
#     model=model,
#     max_len=max_len,
#     batch_size=8,
#     data_name="new_data_mitigations",
#     corpus_embedding_available=False,
